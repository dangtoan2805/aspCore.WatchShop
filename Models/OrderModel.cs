...
namespace aspCore.WatchShop.Models
{
    public class OrderModel
    {
        ...
        public OrderModel(watchContext context, IMapper mapper, IMemoryCache cache)
        {
        ...
        private OrderVM ConvertOrderVM(Order order)
        {
            ...
            obj.Total = (int)((sum * (1 - order.BillPromotion)) * (1 + order.Tax));
            return obj;
        }
        ...
        {
            var data = _db.GetOrders(DateTime.Parse(start), DateTime.Parse(end));
            if (data == null) return null;
            List<OrderVM> orders = new List<OrderVM>();
            data.ForEach(item => orders.Add(ConvertOrderVM(item)));
            return orders;
        ...
        public Tuple<int, int, int, int> GetCountStatus()
        {
            ...
            data.ForEach(item =>
            {
                ...
                total = total + (int)((sum * (1 - item.BillPromotion)) * (1 + item.Tax));
            });
    ...
    public class OrderDetailVM
    {
        ...
        public void AddProductOrder(List<ProductOrder> orders)
        {
            ...
            this.Total = (int)((sum * (1 - this.BillPromotion)) * (1 + this.Tax));
        }
    }
