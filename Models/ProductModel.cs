...
namespace aspCore.WatchShop.Models
{
    public class ProductModel
    {
        ...
        public List<ProductVM> GetAllProduct()
        {
            ...
            {
                if (!_cache.TryGetValue(cateKey, out products))
                {
                    products = ConvertListProduct();
                    if (products == null) return null;
                    _cache.Set(cateKey, products, DateTime.MaxValue);
        ...
        public List<ProductVM> ConvertListProduct()
        {
            List<Product> rawData = _db.GetProducts();
            if (rawData == null) return null;
            List<ProductVM> products = new List<ProductVM>();
            List<ProductPromotion> proms = GetProductsPromotion();
            rawData.ForEach(item =>
            {
                ProductVM obj = _mapper.Map<ProductVM>(item);
                int index = proms.FindIndex(prom => prom.ProductID == obj.ID);
                if (index != -1) obj.Promotion = proms[index].Discount * 100;
                products.Add(obj);
            });
            return products;
        }

        public List<ProductPromotion> GetProductsPromotion()
        {
            List<Promotion> proms = _db.GetProductsPromotion(DateTime.Now);
            List<ProductPromotion> products = new List<ProductPromotion>();
            if (proms == null) return null;
        ...
        public List<ProductVM> GetProductByCate(int id)
        {
        ...
        {
            var data = _mapper.Map<ProductDetailVM>(_db.GetProductDetail(id));
            return data;
        }
        ...
        {
            return GetAllProduct().Where(item => item.Name.Contains(key)).ToList();
        }
    }
